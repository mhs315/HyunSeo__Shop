...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        // 새로운 테이블 생성
        /* db이름은 MONEYBOOK이고, 자동으로 값이 증가하는 _id 정수형 기본키 컬럼과
        item(메뉴) 문자열 컬럼, price(가격) 정수형 컬럼, create_at 문자열 컬럼으로 구성된 테이블을 생성. */
        db.execSQL("CREATE TABLE MONEYBOOK (_id INTEGER PRIMARY KEY AUTOINCREMENT, item TEXT, price INTEGER, create_at TEXT);");
    }

    // DB 업그레이드를 위해 버전이 변경될 때 호출되는 함수
    //SQLiteOpenHelper를 상속받는 경우 생성자와 onCreate, onUpgrade 함수가 필수적으로 추가되어야 한다.
    ...
    void insert(String create_at, String item, int price) {
        // 읽고 쓰기가 가능하게 DB 열기
